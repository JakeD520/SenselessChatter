...
class SupabaseFlowRepository(
    private val client: SupabaseClient
) : FlowRepository {

    override suspend fun listFlows(): List<Flow> =
        client.from("flows")
            .select()
            .decodeList<FlowRow>()
            .map { it.toModel() }
    ...
    override suspend fun sendRipple(flowId: String, alias: String, text: String): Ripple {
        val inserted = client.from("ripples").insert(
            RippleInsert(
                flowId = flowId,
                senderUserId = alias,
                text = text,
    ...
    @Serializable
    private data class RippleInsert(
        @SerialName("flow_id") val flowId: String,
        @SerialName("sender_user_id") val senderUserId: String,
        val text: String? = null,
        @SerialName("media_url") val mediaUrl: String? = null
